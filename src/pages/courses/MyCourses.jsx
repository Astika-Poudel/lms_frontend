...
const MyCourses = () => {
    ...
    if (error) return (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded my-4">
            <p>{error}</p>
            <button onClick={() => { setError(null); fetchEnrolledCourses(); }} className="bg-red-500 text-white px-4 py-2 rounded mt-2 hover:bg-red-600">Try Again</button>
        </div>
    );
    if (enrolledCourses.length === 0) return (
        <div className="text-center py-10">
            ...
            <Link to="/student/course/all" className="bg-[#134e4a] text-white px-6 py-3 rounded-lg hover:bg-[#0c3c38]">Browse Courses</Link>
        </div>
    );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {enrolledCourses.map((course) => (
                    <div key={course._id} className="bg-white rounded-lg shadow-md overflow-hidden">
                        <img src={`${LMS_Backend}/${course.image}`} alt={course.title} className="w-full h-48 object-cover" />
                        <div className="p-4">
                            <h2 className="text-xl font-semibold mb-2">{course.title}</h2>
                            <p className="text-gray-600 mb-4 line-clamp-2">{course.description}</p>
                            <div className="flex items-center text-sm text-gray-500 mb-2">
                                <FaBook className="mr-2" />
                                <span>{(course.beginnerLectures?.length || 0) + (course.advancedLectures?.length || 0)} Lectures</span>
                            </div>
                            <div className="flex items-center text-sm text-gray-500 mb-4">
                                <FaClock className="mr-2" />
                                <span>{course.duration} months</span>
                            </div>
                            <Link to={`/student/course/progress/${course._id}`} className="block w-full bg-[#134e4a] text-white text-center py-2 rounded hover:bg-[#0c3c38]">Continue Learning</Link>
                        </div>
                    </div>
                ))}
            </div>
